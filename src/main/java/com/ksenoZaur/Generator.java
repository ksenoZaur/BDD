...
public class Generator {
    ...
    public String generate(int currentLanguage, int currentPattern) {
        ...
        switch (currentPattern) {
            ...
            default:
                switch (currentLanguage){
                    case 0:
                        return "import java.util.List;import java.util.ArrayList;interface Graphic {   " +
                                " public void print();}class CompositeGraphic implements Graphic {   " +
                                " private List<Graphic> mChildGraphics = new ArrayList<Graphic>();    " +
                                "public void print() {        for (Graphic graphic : mChildGraphics) {            " +
                                "graphic.print();        }    }    public void add(Graphic graphic) {        " +
                                "mChildGraphics.add(graphic);    }    public void remove(Graphic graphic) {        " +
                                "mChildGraphics.remove(graphic);    }}class Ellipse implements Graphic {   " +
                                " public void print() {        System.out.println(\"Ellipse\");    }}public class " +
                                "Program {    public static void main(String[] args) {        Ellipse ellipse1 = " +
                                "new Ellipse();        Ellipse ellipse2 = new Ellipse();        Ellipse ellipse3 = new " +
                                "Ellipse();        Ellipse ellipse4 = new Ellipse();        CompositeGraphic graphic = " +
                                "new CompositeGraphic();        CompositeGraphic graphic1 = new CompositeGraphic();        " +
                                "CompositeGraphic graphic2 = new CompositeGraphic();        graphic1.add(ellipse1);        " +
                                "graphic1.add(ellipse2);        graphic1.add(ellipse3);        graphic2.add(ellipse4);" +
                                "        graphic.add(graphic1);        graphic.add(graphic2);        graphic.print();    }}";
                    case 1:
                        return "\uFEFF  class MainApp  {    static void Main()    {      Composite root = new " +
                                "Composite(\"root\");       root.Add(new Leaf(\"Leaf A\"));      root.Add(new " +
                                "Leaf(\"Leaf B\"));       Composite comp = new Composite(\"Composite X\");       " +
                                "comp.Add(new Leaf(\"Leaf XA\"));      comp.Add(new Leaf(\"Leaf XB\"));      " +
                                "root.Add(comp);      root.Add(new Leaf(\"Leaf C\"));       Leaf leaf = new " +
                                "Leaf(\"Leaf D\");      root.Add(leaf);      root.Remove(leaf);       " +
                                "root.Display(1);       Console.Read();    }  }  abstract class Component  {   " +
                                " protected string name;     public Component(string name)    {      this.name = name;  " +
                                "  }    public abstract void Display(int depth);  }   class Composite : Component  {    " +
                                "private List<Component> children = new List<Component>();     // Constructor    public " +
                                "Composite(string name) : base(name)    {      }     public void Add(Component component) " +
                                "   {      children.Add(component);    }     public void Remove(Component component)    {  " +
                                "    children.Remove(component);    }     public override void Display(int depth)    {   " +
                                "   Console.WriteLine(new String('-', depth) + name);       foreach (Component component in" +
                                " children)      {        component.Display(depth + 2);      }    }  }   class Leaf : Component" +
                                "  {    public Leaf(string name) : base(name)    {      }     public override void Display(int depth)" +
                                "    {      Console.WriteLine(new String('-', depth) + name);    }  }";
                    default:
                        return "\uFEFFfunction Component() {    this.name = '';" +
                                "    this.value = 0;    this.execute = function ()" +
                                " { };}function Leaf(name, value) {    this.name = " +
                                "name;    this.value = value;    this.execute = function" +
                                " () {        return this.value;    };}Leaf.prototype = " +
                                "Object.create(Component.prototype);Leaf.prototype.constructor = " +
                                "Leaf;function Composite(name) {    var self = this;    var children = [];    " +
                                "this.name = name;    this.add = function (component) {        children.push(component);    " +
                                "};    this.remove = function (componentName) {        var newChildren = [];        " +
                                "children.forEach(function (component) {            if (component.name !== componentName) " +
                                "{                newChildren.push(component);            }        });" +
                                "        children = newChildren;    };    this.execute = function () {" +
                                "        children.forEach(function (component) {            " +
                                "self.value += component.execute();        });        return self.value;    }" +
                                ";}Composite.prototype = Object.create(Component.prototype);Composite.prototype." +
                                "constructor = Composite;var kitchen = new Composite('Кухня');kitchen.add( " +
                                "new Leaf('Верхняя секция', 5200) );kitchen.add( new Leaf('Верхняя двойная секция', " +
                                "10000) );kitchen.add( new Leaf('Нижняя секция', 4500) );kitchen.add( new Leaf('Нижняя " +
                                "угловая секция', 7800) );var equipment = new Composite('Техника');equipment.add( new " +
                                "Leaf('Газовая плита', 26400) );equipment.add( new Leaf('Холодильник', 32300) );equipment." +
                                "add( new Leaf('Посудомойка', 21600) );kitchen.add(equipment);console.log('Итого: ' + " +
                                "kitchen.execute() + ' руб.');";
                }
    }
}
